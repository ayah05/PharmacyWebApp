...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["weekDays"])){
        ...
        /*if($_POST["weekDays"] == "Monday"){
            echo "Monday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        } if($_POST["weekDays"] == "Tuesday"){
            echo "Tuesday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        } if($_POST["weekDays"] == "Wednesday"){
            echo "Wednesday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        }
        if($_POST["weekDays"] == "Thursday"){
            echo "Thursday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        } if($_POST["weekDays"] == "Friday"){
            echo "Friday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        } if($_POST["weekDays"] == "Saturday"){
            echo "Saturday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        } if($_POST["weekDays"] == "Sunday"){
            echo "Sunday: ";echo "<br>";
            $weekdays_SUCCESS = "The correct weekdays were set successfully";
            $dontproceed=0;
        }*/
...
if ((isset($_POST["patientID"]))&&(isset($_POST["firstname"]))&&(isset($_POST["familyName"]))&&(isset($_POST["birthdate"]))&&(isset($_POST["gender"]))&&(isset($_POST["maritalStatus"]))&&(isset($_POST["address"]))&&(isset($_POST["postalCode"]))&&(isset($_POST["city"]))&&(isset($_POST["phoneNumber"]))&&(isset($_POST["email"]))&&($dontproceed==0)){
    $patient=array();
    //Transform to JSON the required fields, as long as they are all fulfilled and the optional fields dont have wrong data.
    $patient['resourceType']='Patient';
    $patient['identifier']['id']=$_POST["patientID"];
    $patient['name']['given']=$_POST["firstname"];
    $patient['name']['family']=$_POST["familyName"];
    $patient['birthDate']=$_POST["birthdate"];
    $patient['gender']=$_POST["gender"];
    $patient['maritalStatus']['coding']['display']=$_POST["maritalStatus"];
    $patient['address']['line']=[$_POST["address"]];
    $patient['address']['postalCode']=$_POST["postalCode"];
    $patient['address']['city']=$_POST["city"];
    $patient['telecom']['system']="phone";
    $patient['telecom']['value']=$_POST["phoneNumber"];
    $patient['telecom']['system']=["email"];
    $patient['telecom']['value']=$_POST["email"];

    //Transform to JSON the optional field in case it is set
    if (isset($_POST["secondName"])){
        $patient['name']['given']=[$_POST["firstname"], $_POST["secondName"]];
    }

    $patientJsonData=json_encode($patient);
    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL => 'http://hapi.fhir.org/baseR4/Patient',
        ...
        CURLOPT_POSTFIELDS => $patientJsonData,
    ...
    /* $delim=', "id"';
    $pos=strpos($response,$delim);
    echo $pos;
    $important=substr($response,$pos+strlen($delim)-1,5);
    echo $important; */

    $impt=substr($response,40,7);
}
